...
class geodatrxa :
    ...
    def __init__(self,file=None,echo=False) :
        ...
        self.ReMajor,self.ReMinor,self.Rc,self.phic,self.H,self.prf,self.wavelength=-1,-1,-1,-1,-1,-1,-1
        self.corners=[]
    ...
    def earthRadLatm(self,lat) :
        ''' use radius at the center of the image '''
        N = (self.ReMajor**2.0) / np.sqrt(( self.ReMajor*np.cos(lat))**2.0 + (self.ReMinor*np.sin(lat))**2.0 )
        N = N*1000.
        x = N*np.cos(lat)
    ...
    def isSouth(self) :
        """ Return True if right looking pass - error if lookdir  does not exist or has bad value """
    ...
    def isRightLooking(self) :
        """ Return True if right looking pass - error if lookdir  does not exist or has bad value """
        if len(self.lookdir) == 0 or not (self.lookdir.lower() in ['left','right']) :
            print('geodatrxa.isDescending : no ascdesc value given')
            exit()
        return True if self.lookdir.lower() == 'right' else False

    def isDescending(self) :
        """ Return True if descending pass - error if ascdesc does not exist """

        if len(self.ascdesc) == 0 or not (self.ascdesc.lower() in ['ascending','descending']) :
            print('geodatrxa.isDescending : no ascdesc value given')
            exit()
        ...
        for line in fp :
            ...
            # extract all non-commented data, assuming its in a set order
                ...
                if self.nr == -1 and len(tmp)==4:
                    ...
                    if echo : print('nr,na,nlr,nla ',self.nr,self.na,self.nlr,self.nla)
                elif self.ReMajor == -1 and (len(tmp)==5 or len(tmp)==6 ):
                        self.ReMajor,self.ReMinor=float(tmp[0]) , float(tmp[1])
                        self.Rc, self.phic, self.H=float(tmp[2]),float(tmp[3]), float(tmp[4])
                        if len(tmp) == 6: self.deltaR=float(tmp[5])
                        if echo : print('ReMajor, ReMinor, Rc, phic, h, deltaR: ',  self.ReMajor, self.ReMinor, self.Rc, self.phic, self.H,self.deltaR)
                elif ncorners < 5 and len(tmp) == 2 :
                    self.corners[ncorners,:]=float(tmp[0]),float(tmp[1])
                    ncorners+=1
                    if ncorners==5 :
                        if echo : print(self.corners)
                elif self.slpRg == -1 and len(tmp)==2 :
                    self.slpRg,self.slpAz=float(tmp[0]),float(tmp[1])
                    if echo : print('Single Look Pix Size (r,a)',  self.slpRg,self.slpAz)
                elif tmp[0]  in ['descending','ascending'] and  len(self.ascdesc) ==0 :
                    self.ascdesc=tmp[0]
                    if echo : print(self.ascdesc)
                elif tmp[0]  in ['right','left'] and  len(self.lookdir) ==0:
                    self.lookdir=tmp[0]
                    if echo : print('Look direction ',self.lookdir)                    
                elif tmp[0]  in ['state'] :
                    if echo : print(tmp[0])
                elif len(tmp)==3 and not timeSet :
                    hour,minute,second=int(tmp[0]), int(tmp[1]),int(float(tmp[2]))
                    microsecond=int((float(tmp[2])-second)*1e6)
                    # this is  a kluge for case where squint time pushes over 24 hour boundary - not a problem in most casese
                    if hour > 23 :
                        hour,minute,second=23,59,59
                    self.datetime= self.midnight.replace(hour=hour,minute=minute,second=second,microsecond=microsecond)
                    self.t0=(self.datetime-self.midnight).total_seconds()
                    if echo : print('Date/Time ',self.datetime)                    
                    timeSet=True
                elif len(tmp)==1 and self.prf==-1 :
                    self.prf=float(tmp[0])
                    if echo : print('Prf ',self.prf)
                elif len(tmp)==1 and self.wavelength==-1 :
                    self.wavelength=float(tmp[0])
                    if echo : print('wavelength ',self.wavelength)                    
                elif len(tmp)==1 and self.nState==-1 :
                    self.nState=int(tmp[0])
                    if echo : print('nState ',self.nState)
                    ...
                    if echo : print('tState ',self.tState)                                        
                elif len(tmp)==1 and self.dTState==-1 :
                    self.dtState=float(tmp[0])
                    if echo : print('dtState ',self.dtState)
                elif nState < self.nState and len(tmp)== 3:
                    if position:
                        self.position[nState,:]=[ float(tmp[0]),float(tmp[1]),float(tmp[2]) ]
                        position=False
                    else :
                        self.velocity[nState,:]=[ float(tmp[0]),float(tmp[1]),float(tmp[2]) ]
                        position=True
                        nState+=1
                elif 'deltaT' in line :
                    self.deltaT=float(tmp[1])
                    if echo : print('deltaT ',self.deltaT)
        if echo : 
            print('Position (x,y,z): \n',self.position), print('Velocity (vx,vy,vz): \n',self.velocity)
        #
        self.stateTime=np.array([self.tState + x*self.dtState for x in range(0,self.nState)])
        # compute near in slp coordates for geocoding 
        self.rNearSLP=self.Rc * 1000.0 - ((self.nr-1)/2) *self.nlr * self.slpRg - (self.nlr-1)* self.slpRg/2.   
    ...
    def printState(self) :
        ...
        for t,pos,vel in zip(self.stateTime,self.position,self.velocity) :
            print('{0:10.4f}  {1:8.1f} {2:8.1f} {3:8.1f}  {4:8.1f} {5:8.1f} {6:8.1f}'.format( t,
    ...
    def setupInterpState(self) :
        ...
        kind='cubic'
        bError=False
        self.fx=interp.interp1d(self.stateTime,self.position[:,0],kind=kind,fill_value=np.nan,bounds_error=bError)
        self.fy=interp.interp1d(self.stateTime,self.position[:,1],kind=kind,fill_value=np.nan,bounds_error=bError)
        self.fz=interp.interp1d(self.stateTime,self.position[:,2],kind=kind,fill_value=np.nan,bounds_error=bError)
        self.fvx=interp.interp1d(self.stateTime,self.velocity[:,0],kind=kind,fill_value=np.nan,bounds_error=bError)
        self.fvy=interp.interp1d(self.stateTime,self.velocity[:,1],kind=kind,fill_value=np.nan,bounds_error=bError)
        self.fvz=interp.interp1d(self.stateTime,self.velocity[:,2],kind=kind,fill_value=np.nan,bounds_error=bError)   
    ...
    def lltoecef(self,lat,lon,zelev) :
        ''' convert llz to ecef'''
        return pyproj.transform(self.llz,self.ecef,lon,lat,zelev,radians=False)
    ...
    def llzPtToRA(self,lat,lon,z,initT=None) :
        ...
        for i in range(0,50) :
            ...
            df=np.dot(dr,vPt)
            C1=-np.dot(vPt,vPt)
            #print(myTime,sPt,vPt,np.sqrt(np.dot(dr,dr)))
           ...
           # print(df/C1,sPt)
            ...
            if np.abs(df/C1) < 1e-5 :
                break
            #print(df/C1,i)
        #print(self.Rc,np.sqrt(np.dot(dr,dr)))
        # correct to near range for multi-look, the sub
        #print(rNear)
       
        r=(np.sqrt(np.dot(dr,dr)) - self.rNearSLP)/self.slpRg
        #print(np.sqrt(np.dot(dr,dr)),r)
        #print(lat,lon,z,r, np.sqrt(np.dot(dr,dr)), np.sqrt(np.dot(sPt,sPt)),  np.sqrt(np.dot(tPt,tPt)), )
