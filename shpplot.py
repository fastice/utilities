...
#----------------------------------------------------------------------------------------------------------------------------------------------------
# class defintion for an object based on a shape file point, polygon, or polyline
#----------------------------------------------------------------------------------------------------------------------------------------------------
class shpplot :
    """ \ngeoimage - object for scalar or velocity PS data + geodat data """
    def __init__(self,xdom=None,ydom=None,xplot=None,y1plot=None,y2plot=None,y3plot=None,
                 e1plot=None,e2plot=None,e3plot=None,dates=None,plotType=None,verbose=True,name=None,labels=None) :
    ...
    def setName(self,name) :
        self.name=name
    #
    # append a point to the list for plotting - allow for up to 3 filelds   
    ...
    def appendPointError(self,newY) :
        if len(newY) <= 0 or len(newY) > 3 :
            myerror('shpplot.appendPointError: Invalide number of y points '+newY)
...
# Unpack markers for label/name 
    ...
    def timeSeriesTrend(self,ax=None,color=None,splitTrendByLabel=False,showPlot=True,order=None,timeRange=None):
        #
        if color is None :
            color='r'
        ...
        # unpack sorted data
            ...
            if len(y1use) >= 2 :
                print(dt)
                print(y1use)
                slope,intercept,r_value,p_value,std_err = stats.linregress(dt,y1use)
                ...
                if ax is None and showPlot :
                    ...
                    ax.plot_date(x1use,yfit,'-',color=pColor,label='$\mathregular{r^2}=$'+str(round(r_value**2,2))+',p='+str(round(p_value,2))+',s='+str(round(slope*365.,1))) 
        if timeRange != None :
    ...
    def timeSeries(self,color='r',yIndex=1,markerSize=8,ax=None,markerStyle='o',lineStyle='',lineWidth=1,useLabel=True,deTrend=False,timeRange=None,order=None,yRange=None):
        ...
        # No label case, just plot the whole series
            ...
            # in theory this should not be needed, but wil work if user specifies a list
            marker,mSize=self.firstElement(markerStyle),self.firstElement(markerSize)
            lStyle,lWidth=self.firstElement(lineStyle),self.firstElement(lineWidth)
            pColor=self.firstElement(color)
            ...
            if deTrend :
                ...
                y1-=yt
            if ax is None :
                p=plt.plot_date(x1,y1,color=pColor,marker=marker,markersize=mSize,linestyle=lStyle,linewidth=lWidth,label=self.name)
            else :
                p=ax.plot_date(x1,y1,color=pColor,marker=marker,markersize=mSize,linestyle=lStyle,linewidth=lWidth,label=self.name)
            ax.set_xlim(timeRange)
        return p
    
    def timeSeriesError(self,color=None,markerSize=None,ax=None,markerStyle=None):
        # modify here later to plot other
        y=self.y1plot
        e=self.e1plot
        if ax is None :
    ...
    def profError(self,index,color=None,markerSize=None,ax=None,markerStyle=None):
        # modify here later to plot other
        y=self.y1plot[index]
        e=self.e1plot[index]
        x=self.xplot
    ...
    def profPlot(self,index,color=None,markerSize=None,ax=None,markerStyle=None,lineStyle=None,lineWidth=None,dateFormat=None,useLabel=None):
        print(lineStyle)
        ...
        if ax is None :
            ...
            #exit()
            p=plt.plot(self.xplot,self.y1plot[index],color=color,marker=markerStyle,markersize=markerSize,linestyle=lineStyle,linewidth=lineWidth,label=lab)
        else :
            print('Label ',self.labels)
            #exit()
            p=ax.plot(self.xplot,self.y1plot[index],color=color,marker=markerStyle,markersize=markerSize,linestyle=lineStyle,linewidth=lineWidth,label=lab)
        return p
        
    #
    # build a time series from tablular data in file
    # time
    # file header
    #dateformat=%Y:%m:%d
    #ndatacol=1
        ...
        for line in fpIn :
            ...
            # process the data
                ...
                if 1 :
                    ...
                    myerror('Invalid date format -- '+tsFormat+'for date'+ dateString + 'from '+line)
                # process data
